...
def plateau(value, threshold=0.1, len_x=3):
    """
...
def exctFeature(value, cell):
    ...
    th1 = (maxv - minv) * 0.25 + minv
    th2 = (maxv - minv) * 0.5 + minv
    th3 = (maxv - minv) * 0.75 + minv

    plno, plen, psatrt = plateau(value, threshold=th1)
    add = [plno, plen, psatrt]
    new.extend(add)

    plno, plen, psatrt = plateau(value, threshold=th2)
    add = [plno, plen, psatrt]
    new.extend(add)

    plno, plen, psatrt = plateau(value, threshold=th3)
    add = [plno, plen, psatrt]
    new.extend(add)
